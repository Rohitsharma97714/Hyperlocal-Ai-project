...
const loginHandler = (model, roleName) => async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: account._id, role: roleName }, process.env.JWT_SECRET, { expiresIn: '1d' });
...
router.put('/profile', protect, async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user._id, role: userRole }, process.env.JWT_SECRET, { expiresIn: '1d' });
